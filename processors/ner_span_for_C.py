    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label in zip(example.text_a, example.labels):
            ...
            if label == 'O':
                ...
                labels.extend([label] + ['I-Chemical'] * (len(word_tokens) - 1))
        ...
        for query_word, query_label in zip(example.query_tokens.split(), example.query_labels.split()):
            ...
            if query_label == 'O':
                ...
                query_labels.extend([query_label] + ['I-Chemical'] * (len(query_word_tokens) - 1))
        ...
        if len(tokens+['[SEP]']+query_tokens) >= max_seq_length - special_tokens_count:
            ...
            if len(tokens) >= max_seq_length - special_tokens_count:
            ...
            elif len(tokens) == max_seq_length - special_tokens_count -1:  # [CLS] + tokens + [SEP] + [SEP]
                tokens = tokens[:(max_seq_length-special_tokens_count-1)]
                start_ids = start_ids[:(max_seq_length-special_tokens_count)]   # 注意这里没有-1
                end_ids = end_ids[:(max_seq_length-special_tokens_count)]
                ...
                query_tokens = query_tokens[:(max_seq_length-special_tokens_count-1-len(tokens))]
                start_ids = start_ids[:(max_seq_length-special_tokens_count)]
                end_ids = end_ids[:(max_seq_length-special_tokens_count)]
...
class BnerProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_text(os.path.join(data_dir, "train.txt")), self._read_query_text(os.path.join(data_dir, "none_train.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_text(os.path.join(data_dir, "dev.txt")), self._read_query_text(os.path.join(data_dir, "none_dev.txt")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_text(os.path.join(data_dir, "test.txt")), self._read_query_text(os.path.join(data_dir, "none_test.txt")), "test")
